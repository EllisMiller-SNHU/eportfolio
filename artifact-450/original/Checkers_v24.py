...
class Checkers:
    ...
    def CompTurnInference(s):
        ...
        engine = CheckersInferenceEngine(s)
        chosen_move, reasoning, score = engine.select_best_move(verbose=True)
...
class CheckersInferenceEngine:
    ...
    def gather_facts(s, move):
        ...
        # Basic facts
        kb['is_capture_move'] = target_tile.isPiece
    ...
    def select_best_move(s, verbose=True):
        ...
        if verbose:
            print("")
            print("=" * 60)
            print("INFERENCE ENGINE ANALYSIS")
            print("=" * 60)
        ...
        for move in moves:
            ...
            if verbose:
                move_str = s.format_move(move)
                print("")
                print("Move: " + move_str)
                print("  Phase: " + kb.get('game_phase', 'unknown'))
                print("  Safe: " + str(kb.get('is_safe', 'unknown')))
                print("  Rules fired:")
                for rule in fired_rules:
                    priority_str = str(rule['priority']).rjust(3)
                    score_str = str(rule['score'])
                    if rule['score'] >= 0:
                        score_str = "+" + score_str
                    print("    [" + priority_str + "] " + rule['name'] + ": " + score_str + " - " + rule['explanation'])
        ...
        if verbose:
            print("")
            print("-" * 60)
            print("SELECTED MOVE: " + s.format_move(chosen_move))
            print("SCORE: " + str(best_score))
            print("REASONING:")
            for rule in best_reasoning:
